...
Array.from(Invoices).forEach(function (raw) {
    ...
    <td class="invoice_id"><a href="/apps/invoices/invoice_overview">#TBS'+ raw.invoice_no + '</a></td>\
...
function ViewInvoice(data) {
    ...
    window.location.assign("/apps/invoices/invoice_overview")
}
...
function EditInvoice(data) {
    ...
    localStorage.setItem("invoice_no", invoice_no);
    window.location.assign("/apps/invoices/invoice_create")
}
        ...
        Swal.fire({
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    invoiceList.remove("invoice_id", `<a href="/apps/invoices/invoice_overview">${ids_array[i]}</a>`);
                }
                document.getElementById("remove-actions").classList.add("d-none");
